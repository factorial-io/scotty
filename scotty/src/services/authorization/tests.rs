...
async fn test_wildcard_scope_permissions() {
    ...
    // Create scopes
    service
        .create_scope("test-scope", "Test scope")
        .await
        .unwrap();
    ...
    service
        .assign_user_role("regular-user", "admin", vec!["test-scope".to_string()])
        .await
        .unwrap();
    assert!(
        service
            .check_permission("regular-user", "test-app", &Permission::View)
            .await,
        "Regular user should see test-app"
    );

    // Now test wildcard scope assignment
    service
        .assign_user_role("wildcard-user", "admin", vec!["*".to_string()])
        .await
        .unwrap();

    assert!(
        service
            .check_permission("wildcard-user", "test-app", &Permission::View)
            .await,
        "Wildcard user should see test-app"
    );
}
...
async fn test_wildcard_vs_specific_scopes() {
    ...
    // Create apps
    service
        .set_app_scopes("app-a", vec!["client-a".to_string()])
        .await
        .unwrap();
    service
        .set_app_scopes("app-b", vec!["client-b".to_string()])
        .await
        .unwrap();
    ...
    service
        .assign_user_role("specific-user", "developer", vec!["client-a".to_string()])
        .await
        .unwrap();
    ...
    assert!(
        service
            .check_permission("wildcard-user", "app-a", &Permission::View)
            .await
    );
    assert!(
        service
            .check_permission("wildcard-user", "app-b", &Permission::View)
            .await
    );
    ...
    assert!(
        service
            .check_permission("specific-user", "app-a", &Permission::View)
            .await
    );
    assert!(
        !service
            .check_permission("specific-user", "app-b", &Permission::View)
            .await
    );
}
