...
4. **Review.** The changes show up in your VCS like any other code change (this repo is primarily driven with `jj`, so no git commit hooks are involved). Inspect the diff, accept by committing, reject by restoring the file. The curator and skills rebuild `ENTRY.md` and `GRAPH.md` after every write, so indices are normally fresh; if you hand-edit, restore, or delete node files yourself, run `npx kenkeep index rebuild` afterwards to bring the indices back in sync. `npx kenkeep doctor` reports when they're stale.
...
- `ENTRY.md`: the entry catalog (whole-tree totals + top-level branch list); injected into every new session. Regenerated by the skills after every write; rebuild manually with `npx kenkeep index rebuild`.
- `GRAPH.md`: full edge listing of nodes; available for the harness to read on demand. Regenerated together with `ENTRY.md`.
