...
- Open [**Wire-format changes must be backward compatible in both directions**](practice-wire-format-changes-must-be-backward-compatible-in-both-directions.md) to learn about: Preflight only gates major.minor, so a patch-level wire change silently breaks a newer scottyctl against an older server. #api #compatibility #scottyctl #gotcha
...
### #compatibility
- Open [**Wire-format changes must be backward compatible in both directions**](practice-wire-format-changes-must-be-backward-compatible-in-both-directions.md) — Preflight only gates major.minor, so a patch-level wire change silently breaks a newer scottyctl against an older server.
### #http
- Open [**File transfers stream over HTTP chunked tar, not WebSocket**](practice-file-transfer-http-tar-streaming.md) — Container file transfer uses HTTP GET/PUT with application/x-tar bodies and bounded, streaming I/O on both ends.
### #map
