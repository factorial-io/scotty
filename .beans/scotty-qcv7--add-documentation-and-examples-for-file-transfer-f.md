...
Documentation delivered as part of OpenSpec change `app-file-transfer`:
- New `app:cp` section in `docs/content/cli.md` covering remote-spec syntax, implicit service resolution, `-` for stdin/stdout, RBAC, transfer size limit, and worked examples (download, upload directory, pipe in/out, implicit service).
- README quick-start example for `app:cp`.
- `scottyctl app:cp --help` long-about covers syntax, pipe mode, implicit service, and includes example invocations.
