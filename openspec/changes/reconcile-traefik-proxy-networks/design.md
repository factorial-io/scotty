...
### Decision 13: Availability is a liveness check, not an "inspect succeeded" check

`connectivity_for`'s `lb_up` argument must mean *the load balancer is running*, not *the load balancer could be inspected*. The distinction is not academic: Docker keeps a container's `NetworkSettings.Networks` populated while it is stopped, so an exited Traefik inspects successfully and still reports every per-app network it was attached to.

```
$ docker inspect traefik --format '{{.State.Status}} {{range $k,$v := .NetworkSettings.Networks}}{{$k}} {{end}}'
exited proxy proxy--simple-nginx proxy--simple-nginx-2 traefik_default
```

Deriving availability from the inspect call's success therefore yields `attached ∋ proxy--<app>` for a Traefik that routes nothing, and every running app reports `Connected` — a green "Routable" pill in front of an app that answers no requests. That is the exact class of silent-unreachability bug this change exists to remove, reintroduced through the indicator meant to reveal it. Found by exercising task 10.4 against a stopped Traefik.

So `traefik_networks` returns the attachment set only when `State.Running == Some(true)`, and treats a non-running container the same as an absent one: `Err`/unavailable, which `connectivity_for` already maps to `LoadBalancerUnavailable` for every app that needs routing. One guard at the single point where Docker state enters the pass, so all three triggers (startup, scheduled, event) inherit it — rather than a check per caller.

The mirror-image gap is in the event filter. `event=start` accelerates only *recovery*; nothing observes *degradation*, so a stopped Traefik keeps the stale `Connected` for up to a full `running_app_check`. The filter therefore also carries `die` (emitted by stop, kill, and crash alike, unlike `stop`, which a `docker kill` or an OOM does not emit). A `die`-triggered pass runs the same cache-based path as a `start`-triggered one and needs no special case: it finds the load balancer unavailable and patches every affected app's connectivity, then broadcasts `AppListUpdated`. Recreation still emits `die` then `start`, which the existing ~2s debounce plus single-flight lock already collapses into one pass — the same coalescing that Decision 2 relies on.
...
- **A stopped container retaining its network attachments makes "unavailable" easy to under-detect** (Decision 13) → availability is gated on `State.Running` at the single point where Traefik's attachment set is read, and the unit tests cover a stopped container that still lists the app's network. The same trap applies to any future liveness inference from an inspect call: a successful inspect says the container exists, nothing more.
- **`die` events fire on every Traefik restart, so a recreate now triggers two passes** (`die` then `start`) → the existing ~2s debounce and single-flight lock collapse them, and the transient `LoadBalancerUnavailable` a `die` pass may publish mid-recreate is *true* at that instant; the following `start` pass restores `Connected` within seconds. Reporting the truthful intermediate state is preferred over suppressing it and risking a stale green pill.
