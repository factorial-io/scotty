...
- **Two sources now write the app cache** (`set_apps` from the check, `update_app` from the event pass) → serialized by the single-flight lock; `update_app` is a per-app replace, so a concurrent `set_apps` can only overwrite an annotation with a fresher one from the same source of truth.
