...
- **Watch Docker events** for the Traefik container starting *and dying*, and reconcile immediately, so the repair window is seconds instead of up to one `running_app_check` interval — and so a load balancer going away is reported as promptly as one coming back, rather than leaving a stale "routable" for up to a full interval. Gated by a new setting `traefik.watch_docker_events`, defaulting to `true`; when disabled, the periodic reconciliation still runs.
- **Treat a load balancer that exists but is not running as unavailable.** Docker retains a stopped container's network attachments, so a merely-inspectable Traefik would otherwise be read as attached and every running app reported as routable while nothing routes — the same silent unreachability this change exists to surface. Availability is gated on the container actually running, at the single point where its attachment set is read.
